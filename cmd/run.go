...
func runRunCmd(cmd *cobra.Command, args []string) error {
	...
	asserter, err := asserter.NewServer(
		wemix.OperationTypes,
		wemix.HistoricalBalanceSupported,
		[]*types.NetworkIdentifier{cfg.Network},
		wemix.CallMethods,
		wemix.IncludeMempoolCoins,
		"",
	)
	if err != nil {
	...
	var client *wemix.Client
	if cfg.Mode == configuration.Online {
		if !cfg.RemoteGwemix {
			g.Go(func() error {
				return wemix.StartGwemix(ctx, cfg.GwemixArguments, g)
			})
		}

		var err error
		client, err = wemix.NewClient(cfg.GwemixURL, cfg.Params, cfg.SkipGwemixAdmin)
		if err != nil {
			return fmt.Errorf("%w: cannot initialize wemix client", err)
		}
		defer client.Close()
	}
	...
	err = g.Wait()
	if SignalReceived {
		return errors.New("rosetta-wemix halted")
	}
